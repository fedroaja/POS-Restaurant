...
function Product() {
  ...
  useEffect(() => {
    (async function(){
      let myUser = await fetch(process.env.REACT_APP_BASE_URL + "/trans/product", {method:"get", credentials:"include"});
  ...
  return (
    <div style={{height:'100%', width: '100%', padding: '3%', overflow: 'auto'}}>
      <Container>
        ...
        <Row>
          <Col>
            <Button variant="primary" style={{marginRight:'2%'}} onClick={() => setModalShow(true)}>
              <AddCircleOutlineIcon/> Add Product
            </Button>
            <Button variant="warning" onClick={HandleFilter}><FilterAltIcon/> Filter</Button>
          </Col>
        </Row>
        <Row>
          <Col>
            {
              isLoad ? "Loading..." :
              <TableComp col={columns} data={data}></TableComp>
            }
          </Col>
        </Row>
        <hr></hr>
        <Row>
          <Col>
            <Button variant="primary" style={{marginRight:'2%'}} onClick={() => setModalShowCtg(true)}>
              <AddCircleOutlineIcon/> Add Category
            </Button>
            <Button variant="warning" onClick={HandleFilterCtg}><FilterAltIcon/> Filter</Button>
          </Col>
        </Row>
        <Row>
          <Col>
            {
              isLoad ? "Loading..." :
              <TableComp col={columnsCtg} data={dataCtg}></TableComp>
            }
          </Col>
        </Row>
